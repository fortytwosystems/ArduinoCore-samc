...
if((BOOT_USART_MODULE == SERCOM6) || (BOOT_USART_MODULE == SERCOM7))
	MCLK->APBDMASK.reg |= BOOT_USART_BUS_CLOCK_INDEX ;
else
	MCLK->APBCMASK.reg |= BOOT_USART_BUS_CLOCK_INDEX ;

	/* Set GCLK_GEN0 as source for GCLK_ID_SERCOMx_CORE */
  GCLK->PCHCTRL[BOOT_USART_PER_CLOCK_INDEX].reg = GCLK_PCHCTRL_GEN_GCLK0 | GCLK_PCHCTRL_CHEN ;
  ...
  while ( GCLK->SYNCBUSY.bit.GENCTRL0)
  {
	...
	NVIC_EnableIRQ(BOOT_USART_IRQn);
	NVIC_SetPriority(BOOT_USART_IRQn, 1);
