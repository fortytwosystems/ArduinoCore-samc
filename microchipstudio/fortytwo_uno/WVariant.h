...
// Definitions for TC/TCC channels
...
// GCLK/CCL Enable (1 bit: 0=enabled, 1=disabled) | GCLK (3 bits: 0-7) | CCL (4 bits: 2 for CCL number, 2 for pin)
...
typedef enum _EPioType
{
  ...
  PIO_PCC=18,                         /* The pin is controlled by the PCC peripheral (parallel capture controller on D51 only). */
  PIO_AC_GCLK=19,                     /* The pin is controlled by the AC_GCLK peripheral (analog comparator / generic clock). */
  PIO_CCL=20,                         /* The pin is controlled by the CCL (configurable custom logic) peripheral (I/O). */
...
typedef struct _PinDescription
{
  ...
  EPWMChannel     ulTCChannel ;	            // Must be 8 bits
  EAnalogChannel  ulADCChannelNumber ;	    // Must be 8 bits
  EExt_Interrupts ulExtInt ;	              // Must be 8 bits
...
// Reserved                       (0x27U)
#define GCM_AC                    (0x28U)
#define GCM_SERCOM6_CORE          (0x29U)
#define GCM_SERCOM7_CORE          (0x2AU)
#define GCM_TC5                   (0x2BU)
#define GCM_TC6                   (0x2CU)
#define GCM_TC7                   (0x2DU)
