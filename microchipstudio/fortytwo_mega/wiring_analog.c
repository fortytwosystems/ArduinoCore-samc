...
const uint8_t timerClockIDs[] = 
{
  ...
  GCM_TC7,
   ...
   | defined(TIMER_122Hz) | defined(TIMER_105Hz) | defined(TIMER_81Hz) | defined(TIMER_61Hz) | defined(TIMER_31Hz))
...
void analogReference(eAnalogReference mode)
{
  ...
  } else if (mode >= AR_INTREF_1V024) {		// Values starting at AR_INTREF_1V0 are used for the Supply Controller reference (AR_INTREF)
    SUPC->VREF.reg &= ~SUPC_VREF_SEL_Msk;
    SUPC->VREF.reg |= SUPC_VREF_SEL(mode - AR_INTREF_1V024);  // See eAnalogReference typedef in wiring_analog.h. AR_INTREF_1V0 = 6.
    mode = 0;
  }
...
void analogWrite( pin_size_t pin, int value )
{
  ...
  else if ( g_APinDescription[pin].ulTCChannel != NOT_ON_TIMER )
  {
    ...
    if (!timerEnabled[timerIndex]) {
       ...
       */
#if ((!defined(TIMER_732Hz) && !defined(TIMER_187500Hz)))
